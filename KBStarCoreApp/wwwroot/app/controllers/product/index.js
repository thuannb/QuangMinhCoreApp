var productController = function () {
	...
	function registerEvents() {
		...
		$("#fileInputImage").on('change', function () {
			...
			$.ajax({
				...
				success: function (path) {
					$('#txtImageM').val(path);
					kbstar.notify('Upload image succesful!', 'success');

				},
				error: function () {
					kbstar.notify('There was error uploading files!', 'error');
				}
			});
		});
		...
		$('#btn-export').on('click', function () {
			$.ajax({
				...
				error: function () {
					kbstar.notify('Has an error in progress', 'error');
					kbstar.stopLoading();
				}
			});
		});
	...
	function saveProduct(e) {
		if ($('#frmMaintainance').valid()) {
			...
			$.ajax({
				...
				success: function (response) {
					kbstar.notify('Update product successful', 'success');
				...
				error: function () {
					kbstar.notify('Has an error in save product progress', 'error');
					kbstar.stopLoading();
				}
			});
			return false;
		...
		kbstar.confirm('Are you sure to delete?', function () {
			$.ajax({
				...
				error: function (status) {
					kbstar.notify('Has an error in delete progress', 'error');
					kbstar.stopLoading();
				}
			});
		});
	...
	function initTreeDropDownCategory(selectedId) {
		$.ajax({
			...
			success: function (response) {
				...
				var arr = kbstar.unflattern(data);
	...
	function loadCategories() {
		$.ajax({
			...
			error: function (status) {
				console.log(status);
				kbstar.notify('Cannot loading product category data', 'error');
			}
		});
	}
	...
	function loadData(isPageChanged) {
		...
		$.ajax({
			...
			success: function (response) {
				...
				$.each(response.Results, function (i, item) {
					render += Mustache.render(template, {
						...
						Price: kbstar.formatNumber(item.Price, 0),
						CreatedDate: kbstar.dateTimeFormatJson(item.DateCreated),
			...
			error: function (status) {
				...
				kbstar.notify('Cannot loading data', 'error');
			}
		});
	}
